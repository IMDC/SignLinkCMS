...
if (isset($_POST['submit']) || isset($_POST['submitImg.x'])) {
	$this_login = mysqli_real_escape_string($_POST['login']);
  $this_password = mysqli_real_escape_string($_POST['password']);
...
if (isset($this_login, $this_password)) {
  ...
  */
  $sltok = "signlinkcms";
  $sql = "SELECT member_id, login, bl_pass from membersCopy where login='$this_login' and AES_DECRYPT(CONCAT('$sltok', '$this_login'), SHA1('$this_password') = (select bl_pass from membersCopy where login='$this_login')";  

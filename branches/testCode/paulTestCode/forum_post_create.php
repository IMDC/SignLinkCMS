...
require_once(INCLUDE_PATH . 'htmLawed/htmLawed.php');
$config = array("safe"=>1);
if (isset($_REQUEST['p'])) {
...
if (isset($_POST['cancel'])) {
	...
	if (!isset($_SESSION['errors'])) {	
      ...
      else if ( $_POST['subject'] == "text") {
//         $subjecttext = mysqli_real_escape_string($db, htmlspecialchars($_POST['sub-text']));
         $subjecttext = htmLawed($_POST['sub-text'], $config);
         if (empty($subjecttext) || $subjecttext=="") {
		...
		if (empty($_POST['message']) || ( (empty($_FILES['sl1msg-file']['tmp_name']) && empty($_FILES['sl2msg-file']['tmp_name'])) && empty($_FILES['vmsg-file']['tmp_name']) && empty($_POST['msg-text'])) ) {
			...
			$_SESSION['errors'][] = 'You have chosen to post a Signlink message - this requires that you submit two files: a flash file .swf and a .mp4 file.';
		}
      else if ($_POST['message'] == "video") {
			$ext = strtolower(end(explode('.', $_FILES['vmsg-file']['name'])));
			if (!in_array($ext, $filetypes_video)) {
				$_SESSION['errors'][] = 'You have chosen to post a video message - invalid file format.';
			}
		}
      else if ( $_POST['message'] == "text") {
         /***** sanitize this potentially malicious html ****/
//         $messagetext = mysqli_real_escape_string($db, htmlspecialchars($_POST['msg-text']));
         $messagetext = htmLawed($_POST['msg-text'], $config);
         
         if ( empty($messagetext) || $messagetext=="" ) {
	...
	if (!isset($_SESSION['errors'])) {
		...
		switch ($_POST['subject']) {
			case 'image':
				$subject = '';
            // escape the alt text
            $subject_alt = htmLawed(mysqli_real_escape_string($db, htmlspecialchars($_POST['isub-alt'])), $config);
				break;
			case 'video':
				$subject = '';
            // escape the alt text
            $subject_alt = htmLawed(mysqli_real_escape_string($db, htmlspecialchars($_POST['vsub-alt'])), $config);
				break;
			case 'text':
            // text may already be escaped from error checking the subject above
            // escape the text
            $subject = htmLawed(mysqli_real_escape_string($db, htmlspecialchars($_POST['sub-text'])), $config);
		...
		switch ($_POST['message']) {
			...
			case 'video':
				$message = '';
				$message_alt = htmLawed(mysqli_real_escape_string($db, htmlspecialchars($_POST['vmsg-alt'])), $config);
				break;
			case 'text':
            // message text may already be escaped from error checks above
				$message = htmLawed(mysqli_real_escape_string($db, htmlspecialchars($_POST['msg-text'])), $config);
				$message_alt = '';
				break;
		}
		...
		if (!$result = mysqli_query($db, $sql)) {
         ...
         // update info for user post count
         updateMemberPostCount(intval($_SESSION['member_id']));
...
$(document).ready(function() {
   $("textarea.tinymce").tinymce({
      ...
      valid_elements : "strong/b,i,ul,li,br,img"
   });
});
</script>

...
tinyMCE.importPluginLanguagePack('custom_media');

var TinyMCE_Custom_MediaPlugin = {
	getInfo : function() {
		return {
			longname : 'custom_media',
			author : 'Moxiecode Systems AB',
			authorurl : 'http://tinymce.moxiecode.com',
			infourl : 'http://wiki.moxiecode.com/index.php/TinyMCE:plugins/custom_media',
	...
	initInstance : function(inst) {
		...
		if (!tinyMCE.settings['media_skip_plugin_css'])
			tinyMCE.importCSS(inst.getDoc(), tinyMCE.baseURL + "/plugins/custom_media/css/content.css");
	},

	getControlHTML : function(cn) {
		switch (cn) {
			case "custom_media":
				return tinyMCE.getButtonHTML(cn, 'lang_media_desc', '{$pluginurl}/images/media.gif', 'mceCustom_Media');
		}
	...
	execCommand : function(editor_id, element, command, user_interface, value) {
		// Handle commands
		switch (command) {
			case "mceCustom_Media":
				tinyMCE.openWindow({
						file : '../../plugins/custom_media/custom_media.htm',
	...
	cleanup : function(type, content, inst) {
		...
		switch (type) {
			case "insert_to_editor":
				...
				content = content.replace(/<script[^>]*>\s*write(Flash|ShockWave|WindowsCustom_Media|QuickTime|RealCustom_Media)\(\{([^\)]*)\}\);\s*<\/script>/gi, '<img class="mceItem$1" title="$2" src="' + img + '" />');
			...
			case "insert_to_editor_dom":
				...
				for (i=0; i<nl.length; i++) {
					if (/mceItem(Flash|ShockWave|WindowsCustom_Media|QuickTime|RealCustom_Media)/.test(nl[i].className)) {
				...
				for (i=0; i<nl.length; i++) {
					...
					switch (ci) {
						case 'clsid:d27cdb6e-ae6d-11cf-96b8-444553540000':
							nl[i].parentNode.replaceChild(TinyMCE_Custom_MediaPlugin._createImg('mceItemFlash', d, nl[i]), nl[i]);
							break;

						case 'clsid:166b1bca-3f9c-11cf-8075-444553540000':
							nl[i].parentNode.replaceChild(TinyMCE_Custom_MediaPlugin._createImg('mceItemShockWave', d, nl[i]), nl[i]);
							break;

						case 'clsid:6bf52a52-394a-11d3-b153-00c04f79faa6':
						case 'clsid:22d6f312-b0f6-11d0-94ab-0080c74c7e95':
						case 'clsid:05589fa1-c356-11ce-bf01-00aa0055595a':
							nl[i].parentNode.replaceChild(TinyMCE_Custom_MediaPlugin._createImg('mceItemWindowsCustom_Media', d, nl[i]), nl[i]);
							break;

						case 'clsid:02bf25d5-8c17-4b23-bc80-d3488abddc6b':
							nl[i].parentNode.replaceChild(TinyMCE_Custom_MediaPlugin._createImg('mceItemQuickTime', d, nl[i]), nl[i]);
							break;

						case 'clsid:cfcdaa03-8be4-11cf-b84b-0020afbbccfa':
							nl[i].parentNode.replaceChild(TinyMCE_Custom_MediaPlugin._createImg('mceItemRealCustom_Media', d, nl[i]), nl[i]);
							break;
					}
				}
				...
				for (i=0; i<nl.length; i++) {
					switch (tinyMCE.getAttrib(nl[i], 'type')) {
						case 'application/x-shockwave-flash':
							TinyMCE_Custom_MediaPlugin._createImgFromEmbed(nl[i], d, 'mceItemFlash');
							break;

						case 'application/x-director':
							TinyMCE_Custom_MediaPlugin._createImgFromEmbed(nl[i], d, 'mceItemShockWave');
							break;

						case 'application/x-mplayer2':
							TinyMCE_Custom_MediaPlugin._createImgFromEmbed(nl[i], d, 'mceItemWindowsCustom_Media');
							break;

						case 'video/quicktime':
							TinyMCE_Custom_MediaPlugin._createImgFromEmbed(nl[i], d, 'mceItemQuickTime');
							break;

						case 'audio/x-pn-realaudio-plugin':
							TinyMCE_Custom_MediaPlugin._createImgFromEmbed(nl[i], d, 'mceItemRealCustom_Media');
							break;
					}
				}
				break;
			...
			case "get_from_editor":
				...
				while ((startPos = content.indexOf('<img', startPos+1)) != -1) {
					...
					attribs = TinyMCE_Custom_MediaPlugin._parseAttributes(content.substring(startPos + 4, endPos));

					// Is not flash, skip it
					if (!/mceItem(Flash|ShockWave|WindowsCustom_Media|QuickTime|RealCustom_Media)/.test(attribs['class']))
						continue;
					...
					if (!tinyMCE.getParam('media_use_script', false)) {
						switch (attribs['class']) {
							...
							case 'mceItemWindowsCustom_Media':
						...
						embedHTML = TinyMCE_Custom_MediaPlugin._getEmbed(ci, cb, mt, pl, attribs);
					} else {
						// Use script version
						switch (attribs['class']) {
							case 'mceItemFlash':
	...
	handleNodeChange : function(editor_id, node, undo_index, undo_levels, visual_aid, any_selection) {
		...
		do {
			if (node.nodeName == "IMG" && /mceItem(Flash|ShockWave|WindowsCustom_Media|QuickTime|RealCustom_Media)/.test(tinyMCE.getAttrib(node, 'class'))) {
...
tinyMCE.addPlugin("custom_media", TinyMCE_Custom_MediaPlugin);
